...
class DeviceController(Generic[I, O]):
    """Controller class that orchestrates the streaming daemon thread."""
    ...
    def __hash__(self) -> int:
        """
        Compute object hash. __hash__ and __eq__ are used to determine equality of two DeviceController instances.

        Equivelant in this case means that all of the underlying dependencies (instance IDs,
    ...
    def get_data_for_hash_and_eq(self) -> Tuple[str, ...]:
        """
        Enumerate IDs and other data that are what determine whether or not this controller is equal to another.
    ...
    def start_streaming(self) -> None:
        """Start the worker thread that's responsible for collecting the video frames from the device."""
        with self.lock:
            if self.is_active:
    ...
    def stop_streaming(self) -> None:
        """Stop and releases the video stream."""
    ...
    def _run(self) -> None:
        """
        Background processing method responsible for orchestrating device and dependencies.
